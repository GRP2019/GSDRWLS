...
def compute_random_walk(model, probe_feature, gallery_feature, i, rerank_topk, alpha): 
    ...
    total_grp_num = len(model)*len(model)
    for h in range(len(model)):
        g_g_score = model[h](Variable(gallery_feature[:,int(h*count):int((h+1)*count), :, :].contiguous().cuda()),#volatile=True
                            Variable(gallery_feature[:,int(h*count):int((h+1)*count), :, :].contiguous().cuda()))
        for j in range(len(model)):
            p_g_score = model[j](Variable(probe_feature[i][int(j*count):int((j+1)*count), :, :].unsqueeze(0).contiguous().cuda()),
                                Variable(gallery_feature[:,int(j*count):int((j+1)*count), :, :].contiguous().cuda()))
    ...
    outputs = torch.cat(outputs, 0).view(total_grp_num , -1 ,2)
...
class CascadeEvaluator(object):
    ...
    def evaluate(self, data_loader, query, gallery, alpha, cache_file=None, rerank_topk=75, second_stage=True, dataset=None, save_dir=''):
        with torch.no_grad():
